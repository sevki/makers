...
/// Escape a shell command for a Bazel `genrule` string.
///
/// Bazel expands `$(...)` forms in `cmd` before invoking the shell. Preserve
/// the automatic variables Bazel and make share, but quote make-only function
/// calls such as `$(call ...)` so Bazel does not reject the rule at analysis
/// time. Bare automatic variables (`$@`, `$<`) need no special handling.
fn bazel_cmd_escape(s: &str) -> String {
    ...
    while let Some(offset) = rest.find("$(") {
        ...
        let expansion = &rest[offset + 2..];
        let bazel_expansion = ["@D)", "@F)", "RULEDIR)", "location ", "locations "]
            .iter()
            .any(|prefix| expansion.starts_with(prefix));
        out.push_str(if bazel_expansion { "$(" } else { "$$(" });
        rest = expansion;
    }
    out.push_str(rest);
    out
...
mod tests {
    ...
    #[test]
    fn bazel_commands_quote_make_only_expansions() {
        assert_eq!(
            bazel_cmd_escape("$(call if_changed,objcopy) $@ $< $(@D)"),
            "$$(call if_changed,objcopy) $@ $< $(@D)"
        );
    }
