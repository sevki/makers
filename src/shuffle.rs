...
/// A dependency-graph edge that can be shuffled: `Dep` for a file's
/// prerequisites, `GoalDep` for the top-level goal chain.
pub trait ShuffleNode: NextLinked {
...
impl_shuffle_node!(Dep);
impl_shuffle_node!(GoalDep);

fn random_shuffle<T>(slice: &mut [*mut T]) {
    let len = slice.len();
    if len <= 1 {
        return;
    }
