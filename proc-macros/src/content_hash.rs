...
pub fn add_trait_bounds(mut generics: Generics) -> Generics {
    for param in &mut generics.params {
        if let GenericParam::Type(type_param) = param {
            type_param
                .bounds
                .push(parse_quote!(::make_sys::content_hash::ContentHash));
        }
    }
    generics
}
            ...
            let match_hash_statements = data.variants.iter().enumerate().map(|(i, v)| {
                ...
                match &v.fields {
                    ...
                    Fields::Unit => {
                        ...
                        quote_spanned! {v.span() =>
                            Self::#variant_id => {
                                ::make_sys::content_hash::ContentHash::hash(&#ix, state);
                            }
                        }
                    }
                }
...
fn hash_statements_for_enum_fields<'a>(
    ...
    hash_statements.push(quote! {::make_sys::content_hash::ContentHash::hash(&#ix, state);});
    for (ty, b) in &typed_bindings {
        hash_statements.push(quote_spanned! {b.span() =>
            <#ty as ::make_sys::content_hash::ContentHash>::hash(#b, state);
        });
    }
